...
const EMPTY = 0;
const WHITE = 1;   // игрок (снизу)
const BLACK = 2;   // ИИ (сверху)
...
function newGame() {
  let firstTurn = SETTINGS.firstPlayer;
  ...
  if (G.turn === BLACK) setTimeout(aiTurn, 600);
}
...
function rollDice() {
  ...
  setTimeout(() => {
    ...
    if (G.validMoves.length === 0) {
      ...
      updateStatus('Нет доступных ходов. Нажмите «Пропустить ход».');
    } else {
      document.getElementById('passBtn').disabled = true;
    }
    render();

    if (G.turn === BLACK && !G.gameOver) {
...
function setRollBtn(active) {
  ...
  btn.disabled = !active || G.gameOver || G.turn === BLACK;
}
...
function renderDice() {
  ...
  d1.classList.toggle('active', has0 && G.turn === WHITE && !G.awaitingRoll);
  d2.classList.toggle('active', has1 && G.turn === WHITE && !G.awaitingRoll);
}
...
function renderBar() {
  ...
  function buildSlots(el, clr) {
    ...
    const selCol = (clr === WHITE && typeof G.selected === 'object' && G.selected && G.selected.bar)
      ? G.selected.col : -1;
...
function updateStatus(customText) {
  ...
  if (!text) {
    if (G.gameOver) {
      ...
      text = G.turn === WHITE ? '<span class="who">Ваш ход.</span> Бросьте кубики.' : '<span class="who">Ходят чёрные.</span> ИИ бросает кубики…';
    } else {
      const remain = G.dice.filter(d => !d.used).map(d => `${d.value}`).join(' + ');
      text = `<span class="who">${G.turn === WHITE ? 'Ход игрока' : 'ИИ ходит'}.</span> Осталось: ${remain}`;
      if (G.bar[G.turn].length > 0) {
        text += ' (есть шашка на баре — можно вернуть или ходить другой)';
      }
    }
  }
  st.innerHTML = text;
...
function showWinner(clr) {
  ...
  document.getElementById('winSub').textContent = clr === WHITE
    ? 'Все ваши шашки на половине соперника'
    : 'Все шашки ИИ на вашей половине';
...
function onCanvasClick(ev) {
  if (G.gameOver || G.turn !== WHITE || G.awaitingRoll || G.aiThinking) return;
...
function onBarClick(ev) {
  if (G.gameOver || G.turn !== WHITE || G.awaitingRoll) return;
  if (G.bar[WHITE].length === 0) return;
  ...
  if (!G.bar[WHITE].includes(col)) return;
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('rollBtn').addEventListener('click', () => {
    if (G.turn === WHITE && G.awaitingRoll) rollDice();
  });
